...
def memory(liste, index_input, relative_base_input ,direction):
    index=index_input
...
def update_pos(robot_position, orientation):
    if orientation == 1:
...
#     response, index_input, relative_base_input = memory(liste, index_input, relative_base_input, orientation)

#     if response == 0:
#         blocked_pos = update_pos(robot_position, orientation).copy()
#         map_room[blocked_pos[0]][blocked_pos[1]] = 0
#         orientation = update_orientation(orientation)

#     elif response == 2:
#         robot_position = update_pos(robot_position, orientation).copy()
#         map_room[robot_position[0]][robot_position[1]] = 1
#         print(counter, robot_position)


#     elif response == 1:
#         robot_position = update_pos(robot_position, orientation).copy()
#         orientation = update_orientation(orientation)
#         map_room[robot_position[0]][robot_position[1]] = 2
...
# with open('day15/map', 'wb') as fp:
#     pickle.dump(map_room, fp)

with open ('day15/map', 'rb') as fp:
    map_room = pickle.load(fp)



for i in range(len(map_room)):
    if 2 in map_room[i]:
        end_point = [i, map_room[i].index(2)]
        map_room[end_point[0]][end_point[1]] = 1

for i in range(len(map_room)):
    for j in range(len(map_room[0])):
        if map_room[i][j]==2:
            map_room[i][j] = 1
        if map_room[i][j]>=1:
            map_room[i][j] = 1

plt.imshow(map_room, cmap="gray")
plt.show()

grid = Grid(matrix=map_room)
