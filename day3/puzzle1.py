def points(path):
    ...
    for move in path:
        if move[0]=="U":
            new_point = (points_list[-1][0], points_list[-1][1]+int(move[1:]))
        elif move[0]=="D":
            new_point = (points_list[-1][0], points_list[-1][1]-int(move[1:]))
        elif move[0]=="L":
            new_point = (points_list[-1][0]-int(move[1:]), points_list[-1][1])
        elif move[0]=="R":
            new_point = (points_list[-1][0]+int(move[1:]), points_list[-1][1])
        points_list += [new_point]
    return points_list
        

def isCrossingLine(startA, endA, startB, endB):

    if startA[0]==endA[0]:
        if startB[0]==endB[0]:
            return (False,(0,0))
        else:
            crossed = (startA[0]<max(startB[0],endB[0])) and (startA[0]>min(startB[0],endB[0]))
            crossed = crossed and ((startB[1]<max(startA[1],endA[1])) and (startB[1]>min(startA[1],endA[1])))
            return(crossed,(startA[0],startB[1]))

    elif startA[1]==endA[1]:
        if startB[1]==endB[1]:
            return (False,(0,0))
        else:
            return isCrossingLine(startB, endB, startA, endA)


def isCrossingPath(path1, path2):
    crossPoints = []
    points1=points(path1)
    points2=points(path2)
    for k in range(len(points1) - 1):
        for l in range(len(points2) - 1):
            (currentLineCheck, crossPoint) = isCrossingLine(points1[k], points1[k+1], points2[l], points2[l+1])
            if currentLineCheck and not(crossPoint in crossPoint) and not(crossPoint == (0,0)):
                crossPoints += [crossPoint]
    return crossPoints

def closest(points):
    closest = points[0]
    closest_dist = abs(closest[0])+abs(closest[1])
    for point in points:
...
def steps(point,points_list):
    steps = 0
    for k in range(len(points_list)-1):

        currentline=(points_list[k],points_list[k+1])

        if currentline[0][0]==currentline[1][0]:
            if point[0]==currentline[0][0] and (point[1]<=max(currentline[0][1],currentline[1][1])) and (point[1]>=min(currentline[0][1],currentline[1][1])):
                return steps + abs(currentline[0][1]-point[1])

        elif currentline[0][1]==currentline[1][1]:
            if point[1]==currentline[0][1] and (point[0]<=max(currentline[0][0],currentline[1][0])) and (point[0]>=min(currentline[0][0],currentline[1][0])):
                return steps + abs(currentline[0][0]-point[0])

        steps += abs(points_list[k][0] - points_list[k+1][0]) + abs(points_list[k][1] - points_list[k+1][1])


    return steps

def steps_lowest(points,points1,points2):
    lowest = points[0]
    lowest_steps = steps(lowest,points1)+steps(lowest,points2)
    for point in points:
        temp_steps = steps(point,points1)+steps(point,points2)
        if temp_steps < lowest_steps:
            lowest = point
            lowest_steps = temp_steps
            
    return (lowest, lowest_steps)
...
pointslist = isCrossingPath(path1,path2)
print(pointslist)
print(closest(pointslist))

print(steps_lowest(pointslist,points(path1),points(path2)))
